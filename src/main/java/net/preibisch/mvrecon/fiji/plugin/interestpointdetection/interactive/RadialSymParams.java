...
public class RadialSymParams implements Serializable {
	...
	public void printParams( final boolean printIntensityThreshold ) {
		IOFunctions.println("SigmaDoG               : " + sigma);
		IOFunctions.println("ThresholdDoG           : " + threshold);
		IOFunctions.println("anisotropyCoefficient  : " + anisotropyCoefficient);
		IOFunctions.println("useAnisotropyForDoG    : " + useAnisotropyForDoG);
		IOFunctions.println("RANSAC                 : " + RANSAC() );
		IOFunctions.println("MaxError               : " + maxError);
		IOFunctions.println("InlierRatio            : " + inlierRatio);
		IOFunctions.println("supportRadius          : " + supportRadius);
		IOFunctions.println("GaussFitIntensity      : " + gaussFitIntensity);
		//IOFunctions.println("GaussFitLocation       : " + gaussFitLocation);
		if ( printIntensityThreshold )
			IOFunctions.println("intensityThreshold     : " + intensityThreshold);
		IOFunctions.println("min intensity          : " + min);
		IOFunctions.println("max intensity          : " + max);
		IOFunctions.println("autoMinMax             : " + autoMinMax);
		IOFunctions.println("resultsFilePath        : " + resultsFilePath);

		if ( ransacSelection == 2 )
		{
			IOFunctions.println("minNumInliers          : " + minNumInliers);
			IOFunctions.println("nTimesStDev1           : " + nTimesStDev1);
			IOFunctions.println("nTimesStDe             : " + nTimesStDev2);
		}

		IOFunctions.println("bsMethod               : " + bsMethod);
		IOFunctions.println("bsMaxError             : " + bsMaxError);
		IOFunctions.println("bsInlierRatio          : " + bsInlierRatio);
